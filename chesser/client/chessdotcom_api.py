...
def get_player_months_archive(user_name):
    ...
    for archive in archive_list:
        ...
        archives.append(UserArchive(user_name=user_name, date_month=date_month))

    return archives


def get_games_from_monthly_archive(user_name, date_month: date):
    headers = {'user-agent': user_name}
    url = f'{URL_BASE}/{user_name}/games/{date_month.strftime("%Y/%m")}'
    ...
    for game in res.json().get('games'):
        ...
        pgn_list.append(GamePGN(register=utcdate, pgn=pgn))
